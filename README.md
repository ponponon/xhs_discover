...
### 数据导出
- **导出 JSON**：适合程序化处理和数据分析
- **导出 CSV**：适合 Excel 等表格软件打开
- **导出 Excel**：直接生成 .xls 文件，保留中文格式
- **复制链接**：一键复制所有帖子链接到剪贴板

### 批量下载
- **下载封面图**：批量下载所有帖子的封面图片，保存到 `xiaohongshu/` 文件夹
- **下载头像**：批量下载所有发布者的头像图片，保存到 `xiaohongshu/` 文件夹
